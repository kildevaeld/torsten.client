...
function validateConfig(options) {
    if (options == null)
        throw error_1.createError(0, "options");
}
class TorstenClient {
    constructor(options) {
    ...
    create(path, data, options = {}) {
        if (data == null)
            return Promise.reject(error_1.createError(error_1.ErrorCode.NullData, "no data"));
    ...
    list(path, options = {}) {
        ...
        return req.then(getResponse).then(res => res.json()).then(infos => {
            if (infos.message != 'ok')
                return [];
